...
if not results:
    ...
    for i, result in enumerate(results):
        # Initialize ui_state for this result if it doesn't exist
        if i not in st.session_state.results_ui_state:
            st.session_state.results_ui_state[i] = {
                'show_source': -1, # -1 means no source is shown
                'show_other_sources': False
            }
        ui_state = st.session_state.results_ui_state[i]

        # Initialize the expander's state in ui_state based on Streamlit's internal state
        # This needs to happen *before* the st.expander call
        if f"expander_{i}" in st.session_state:
            ui_state['expanded'] = st.session_state[f"expander_{i}"]
        else:
            ui_state['expanded'] = True # Default to collapsed

        with st.expander(f"Result for {result['company_name']} - {result['created_at']}", expanded=ui_state['expanded']):
            # No need to update ui_state['expanded'] inside the expander block anymore,
            # as it's already initialized correctly. Streamlit will update its internal state.

            st.write("**Company:**", result['company_name'])
            st.write("**Query:**", result['query_text'])
            st.write("**Date:**", result['created_at'])

            st.write("**Answer:**")
            try:
                # Try to parse and pretty-print the JSON answer
            ...
            st.write("**Sources:**")
            try:
                sources = json.loads(result['source'])
                if sources:
                    ...
                    view_source_label = "Close Source" if ui_state['show_source'] == 0 else "View Source"
                    if st.button(view_source_label, key=f"view_source_{i}_0"):
                        ui_state['show_source'] = 0 if ui_state['show_source'] != 0 else -1
                        st.rerun()

                    if ui_state['show_source'] == 0:
                        # Initialize checkbox states from st.session_state if they exist, otherwise default
                        initial_crop_view = st.session_state.get(f"crop_{i}_0", False)
                        initial_encompass_view = st.session_state.get(f"encompass_{i}_0", True) # Default to True as in original code

                        crop_view = st.checkbox("Crop to Bounding Box", value=initial_crop_view, key=f"crop_{i}_0")
                        encompass_view = st.checkbox("Encompass Bounding Boxes", value=initial_encompass_view, key=f"encompass_{i}_0")
                    ...
                    if len(sources) > 1:
                        show_other_sources_label = "Hide Other Sources" if ui_state['show_other_sources'] else "Show Other Sources"
                        if st.button(show_other_sources_label, key=f"show_other_sources_{i}"):
                            ui_state['show_other_sources'] = not ui_state['show_other_sources']
                            st.rerun()

                        if ui_state['show_other_sources']:
                            for j, source in enumerate(sources[1:]):
                                st.write(f"**Source {j + 2}:**")
                                if 'llm_evaluation' in source:
                                    st.json(source['llm_evaluation'])

                                view_source_label_other = "Close Source" if ui_state['show_source'] == j + 1 else "View Source"
                                if st.button(view_source_label_other, key=f"view_source_{i}_{j+1}"):
                                    ui_state['show_source'] = j + 1 if ui_state['show_source'] != j + 1 else -1
                                    st.rerun()

                                if ui_state['show_source'] == j + 1:
                                    # Initialize checkbox states from st.session_state if they exist, otherwise default
                                    initial_crop_view_other = st.session_state.get(f"crop_{i}_{j+1}", False)
                                    initial_encompass_view_other = st.session_state.get(f"encompass_{i}_{j+1}", True) # Default to True

                                    crop_view_other = st.checkbox("Crop to Bounding Box", value=initial_crop_view_other, key=f"crop_{i}_{j+1}")
                                    encompass_view_other = st.checkbox("Encompass Bounding Boxes", value=initial_encompass_view_other, key=f"encompass_{i}_{j+1}")
